/*this file rank the individuals without constraints in each generation
...
void ranking(population *pop_ptr)
{
	...
	for (k=0;k<popsize;k++,q=0) {
		...
		for (i=0;i<popsize;i++) { 
			...
			if (pop_ptr->ind_ptr->flag!=1 && pop_ptr->ind_ptr->flag!=0) {//flag check
				...
				for (j=0;j<popsize;j++) { //select the second individual
					if (i!=j) {
						if (pop_ptr->ind[j].flag!=1) {
							...
							/*printf("\n---------------------------------------------\n");
							printf("\nfor the comparation, val= %d", val);
							printf("\nflag of first ind(%d): %d", i+1, pop_ptr->ind[i].flag);
							printf("\nflag of second ind(%d): %d", j+1, pop_ptr->ind[j].flag);*/
						}
					}
					/*printf("\n========================================\n");
					printf("\n at the end of the loop over j:");
					printf("\n the flag of %d th Ind(i): %d", i+1, pop_ptr->ind[i].flag);
					printf("\n the flag of %d th Ind(j): %d", j+1, pop_ptr->ind[j].flag);*/
				...
				/*printf("\n========================================\n");
				printf("\n at the end of the loop of flag check:");
				printf("\n the flag of %d th Ind(i): %d", i+1, pop_ptr->ind[i].flag);
				printf("\n the flag of %d th Ind(j): %d", j+1, pop_ptr->ind[j].flag);*/
	...
	/*printf("\n====================In Ranking.c=======================\n");
	for (i=0;i<popsize;i++) {
		printf("\nIND \tfitness \t\trank \t\tflag ");
		printf("\n%d  \t%.4f %.4f \t\t%d \t\t%d", i+1, pop_ptr->ind[i].fit[0],
			pop_ptr->ind[i].fit[1], pop_ptr->ind[i].rank, pop_ptr->ind[i].flag);
	}
	printf("\nMaxrank of the population is: %d", pop_ptr->maxrank);
	for (i=0;i<pop_ptr->maxrank;i++)
		printf("\nthe rankno at rank %d is : %d", i+1, pop_ptr->rankno[i]);
	printf("\n========================================================\n");*/
	return;
}
