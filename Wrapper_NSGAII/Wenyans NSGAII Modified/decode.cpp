...
void decode(population *pop_ptr)
{
	...
	//Debug decode.c
	//print out decoded values(xchrom1, ychrom1, zchrom1) of chrom1
	/*printf("\n------------------------Decode.c--------------------------------\n");
	for (i=0;i<popsize;i++) {
		printf("\nInd%d \tchrom1 \txchrom1 \tychrom1 \tzchrom1\n", i+1);
		for (j=0;j<no_intevar;j++) {
			printf("DV%d: \t%d  \t%g  \t%g  \t%g  \t%g  \t%g\n",j+1,pop_ptr->ind[i].chrom1[j],
			pop_ptr->ind[i].xchrom1[j],pop_ptr->ind[i].ychrom1[j],pop_ptr->ind[i].zchrom1[j],
	        pop_ptr->ind[i].uchrom1[j],pop_ptr->ind[i].vchrom1[j]);
		}
	}
	printf("\n-------------------------------------------------------------------\n");*/
	return;
