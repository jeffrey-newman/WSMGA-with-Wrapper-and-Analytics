	...
	for (i=0,n=0,y=0;i<popsize/2;i++) {
		...
		if (rnd<=pc) { //crossover will take place
			...
			for (k=0;k<no_intevar;k++) 
				printf(" %d ", pop_ptr2->ind[y].chrom1[k]);
			printf("\n2st Parent is:");
			for (k=0;k<no_intevar;k++) 
				printf(" %d ", pop_ptr2->ind[y+1].chrom1[k]);
			printf("\n1st Child is:");
			for (k=0;k<no_intevar;k++) {
				printf(" %d ", pop_ptr1->ind[n].chrom1[k]);
			}
			printf("\n The tag for 1st child is %d ", pop_ptr1->ind[n].tag);
			printf("\n2st Child is:");
			for (k=0;k<no_intevar;k++) {
				printf(" %d ", pop_ptr1->ind[n+1].chrom1[k]);
			}
			printf("\n The tag for 2nd child is %d ", pop_ptr1->ind[n+1].tag);*/
